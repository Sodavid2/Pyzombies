    ...
    for item in inventory:
        if item.type == "item":
            if item not in counted:
                print("    {} x {} ({}Kg)".format(item.name, inventory.count(item),
                                                round(item.weight * inventory.count(item), 1)))
                counted.append(item)
        elif item.type == "weapon":
            if not item.durability:
                print(
                    "    {} - {}/{} ({}, {}Kg)".format(item.name, item.ammo, item.max_ammo, item.ammo_type, item.weight))
            else:
                print("    {} - {}/{} durability ({}Kg)".format(item.name, item.durability, item.max_durability,
                                                              item.weight))
...
def inspect_health():
    if P.health <= 0:
        print("you died")
        t.sleep(GAME_SPEED)
        print("you survived for {} hours".format((World.time - 360) / 60))
        print("enemies killed:", Statistics.enemies_killed)
        print("items found:", Statistics.items_found)
        print("cheats:", Statistics.cheats)
        t.sleep(GAME_SPEED)
        input()
        exit()
...
def fight(enemy_count=1, enemy_pool=False, location=world_type.locations[0]):
    ...
    if enemy_pool:
        ...
        for i in range(enemy_count):
            enemy = r.choices(location.enemies, location.enemy_weights)
            enemies.append(c.deepcopy(enemy[0]))
            enemies[i].distance = r.randint(20 + (i * 5), 30 + (i * 10)) / 10
